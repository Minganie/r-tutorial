...
Data is typically described regarding three characteristics: where is the ``middle'' of the distribution (center tendency), how ``spread out'' is it (dispersion), and what ``shape'' is it.
	...
	\section{Center tendency measurements}
		...
		While the basic principle is very simple, there are a few subtleties that are worth pointing out about the mean. Let's start with a simple example. We will use Fisher's Iris data set, which comes already bundled in R. Calculating the average is a simple case of calling up the function \code{mean}.
...
[1] 5.843333
		\end{minted}
		There you have it. However, one can also compute a mean from a frequency table, with the following formula:
		
		\[
		\overline{x}=\frac{\sum\limits_{i=1}^k f_i.x_i}{n}
		\]
		
		...where $i$ represents each class, $f_i$ is the frequency of this class and $x_i$ the value of the variable in this class. With our iris data, for example, one could produce a frequency table of sepal length by iris species.
		
		It is also the \emph{center of mass} of the distribution.
